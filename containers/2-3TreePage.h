#ifndef TWOTHREETREEPAGE_H
#define TWOTHREETREEPAGE_H
...
template <typename keyType, typename ObjIDType> class TwoThreeTree;
template <typename keyType, typename ObjIDType> class TwoThreeForwardIterator;
template <typename keyType, typename ObjIDType> class TwoThreeBackwardIterator;

using namespace std;

enum tt_ErrorCode {tt_ok, tt_overflow, tt_underflow, tt_duplicate, tt_nofound, tt_rootmerged};

template <typename keyType, typename ObjIDType>
struct tagTwoThreeObjectInfo
{
       keyType                 key;
       ObjIDType               ObjID;
       long                    UseCounter;
       tagTwoThreeObjectInfo(const keyType     &_key, ObjIDType _ObjID)
               : key(_key), ObjID(_ObjID), UseCounter(0) {}
       tagTwoThreeObjectInfo()                          {}
       operator keyType                         ()     { return key; }
       long                    GetUseCounter() { return UseCounter;    }
};

template <typename keyType, typename ObjIDType>
class TwoThreeTreePage 
{
       friend class TwoThreeTree<keyType, ObjIDType>;
       friend class TwoThreeForwardIterator<keyType, ObjIDType>;
       friend class TwoThreeBackwardIterator<keyType, ObjIDType>;

       typedef TwoThreeTreePage<keyType, ObjIDType>    BTPage;
       typedef tagTwoThreeObjectInfo<keyType, ObjIDType> ObjectInfo;
 ...
 public:
       // Constructor
       TwoThreeTreePage(Size maxKeys, bool unique = true);
       
       TwoThreeTreePage(const TwoThreeTreePage& other);
       TwoThreeTreePage& operator=(const TwoThreeTreePage& other);

       // Move Constructor
       TwoThreeTreePage(TwoThreeTreePage&& other) noexcept;
       TwoThreeTreePage& operator=(TwoThreeTreePage&& other) noexcept;

       // Destructor
       virtual ~TwoThreeTreePage();

       tt_ErrorCode Insert (const keyType &key, const ObjIDType ObjID);
       tt_ErrorCode Remove (const keyType &key, const ObjIDType ObjID);
...
protected:
       Size m_MinKeys; 
       Size m_MaxKeys, m_MaxKeysForChilds; 
       bool m_Unique;
       bool m_isRoot;
       ...
       void  clear ();
       void  CopyFrom(const TwoThreeTreePage& other); 
       ...
       tt_ErrorCode Merge  (Size pos);
       tt_ErrorCode MergeRoot ();
...
Size tt_binary_search(Container& container, Size first, Size last, ObjType &object)
{
...
template <typename Container, typename ObjType>
void tt_insert_at(Container& container, const ObjType &object, Size pos)
{
...
void tt_remove(Container& container, Size pos)
{
       Size size = container.size();
...
template <typename keyType, typename ObjIDType>
TwoThreeTreePage<keyType, ObjIDType>::TwoThreeTreePage(Size maxKeys, bool unique)
...
template <typename keyType, typename ObjIDType>
TwoThreeTreePage<keyType, ObjIDType>::TwoThreeTreePage(const TwoThreeTreePage& other) : m_MaxKeys(other.m_MaxKeys), m_Unique(other.m_Unique), m_KeyCount(0) 
{
    Create();
    CopyFrom(other);
}
...
template <typename keyType, typename ObjIDType>
TwoThreeTreePage<keyType, ObjIDType>& TwoThreeTreePage<keyType, ObjIDType>::operator=(const TwoThreeTreePage& other) 
{
    if(this != &other) { 
        Reset(); 
        CopyFrom(other); 
...
template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::CopyFrom(const TwoThreeTreePage& other) 
{
    ...
    for(Size i = 0; i < other.m_SubPages.size(); ++i) {
        if(other.m_SubPages[i]) {
            m_SubPages[i] = new TwoThreeTreePage(other.m_MaxKeys, other.m_Unique);
...
template <typename keyType, typename ObjIDType>
TwoThreeTreePage<keyType, ObjIDType>::TwoThreeTreePage(TwoThreeTreePage&& other) noexcept 
{
...
template <typename keyType, typename ObjIDType>
TwoThreeTreePage<keyType, ObjIDType>& TwoThreeTreePage<keyType, ObjIDType>::operator=(TwoThreeTreePage&& other) noexcept 
{
    if(this != &other) {
        Reset();
        m_MinKeys = std::exchange(other.m_MinKeys, 0);
...
template <typename keyType, typename ObjIDType>
TwoThreeTreePage<keyType, ObjIDType>::~TwoThreeTreePage()
{
       Reset();
}

template <typename keyType, typename ObjIDType>
tt_ErrorCode TwoThreeTreePage<keyType, ObjIDType>::Insert(const keyType& key, const ObjIDType ObjID)
{
       Size pos = tt_binary_search(m_Keys, 0, m_KeyCount, key);
       tt_ErrorCode error = tt_ok;
       ...
       if( !m_SubPages[pos] ) 
       {
               ::tt_insert_at(m_Keys, ObjectInfo(key, ObjID), pos);
               m_KeyCount++;
               if( Overflow() ) return tt_overflow;
               return tt_ok;
       }
       else
       {
...
template <typename keyType, typename ObjIDType>
bool TwoThreeTreePage<keyType, ObjIDType>::Redistribute1(Size &pos)
{
       if( m_SubPages[pos]->Underflow() )
       {
               ...
               if( nkol > nkor )
                       if( m_SubPages[pos-1]->NumberOfKeys() > m_SubPages[pos-1]->MinNumberOfKeys() )
                               ...
                               if( pos == NumberOfKeys() ) return (--pos, false);
                               else return false;
               else 
                       if( m_SubPages[pos+1]->NumberOfKeys() > m_SubPages[pos+1]->MinNumberOfKeys() )
...
template <typename keyType, typename ObjIDType>
bool TwoThreeTreePage<keyType, ObjIDType>::Redistribute2(Size pos)
{
...
template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::RedistributeR2L(Size pos)  
{
       ...
       {
               ::tt_insert_at(pTarget->m_Keys, m_Keys[pos-1], pTarget->NumberOfKeys()++);
               ::tt_insert_at(pTarget->m_SubPages, pSource->m_SubPages[0], pTarget->NumberOfKeys());

               m_Keys[pos-1] = pSource->m_Keys[0];

               ::tt_remove(pSource->m_Keys    , 0);
               ::tt_remove(pSource->m_SubPages, 0);
               pSource->NumberOfKeys()--;
       }
}

template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::RedistributeL2R(Size pos)
{
       BTPage  *pSource = m_SubPages[pos], *pTarget = m_SubPages[pos+1];
       ...
       while(pSource->GetNumberOfKeys() > pSource->MinNumberOfKeys() &&
                 pTarget->GetNumberOfKeys() < pSource->GetNumberOfKeys() )
       {
               ::tt_insert_at(pTarget->m_Keys, m_Keys[pos], 0);
               ::tt_insert_at(pTarget->m_SubPages, pSource->m_SubPages[pSource->NumberOfKeys()], 0);
...
template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::SplitChild(Size pos)
{
       ...
       ::tt_insert_at(m_Keys, oi2, pos+1);
       ::tt_insert_at(m_SubPages, pChild2, pos+1);
       NumberOfKeys()++;
...
template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::SplitPageInto3(vector<ObjectInfo>& tmpKeys,
                                                vector<BTPage *>  & tmpSubPages,
                                                BTPage* &     pChild1,
                                                BTPage* &     pChild2,
                                                BTPage* &     pChild3,
...
template <typename keyType, typename ObjIDType>
bool TwoThreeTreePage<keyType, ObjIDType>::SplitRoot()
{
...
template <typename keyType, typename ObjIDType>
bool TwoThreeTreePage<keyType, ObjIDType>::Search(const keyType &key, ObjIDType &ObjID)
{
       Size pos = tt_binary_search(m_Keys, 0, m_KeyCount, key);
       if( pos >= m_KeyCount ){
...
template <typename keyType, typename ObjIDType>
template <typename... Args>
void TwoThreeTreePage<keyType, ObjIDType>::InOrder(lpfnForEach<Args...> lpfn, Size level, Args... args)
{
...
template <typename keyType, typename ObjIDType>
template <typename... Args>
void TwoThreeTreePage<keyType, ObjIDType>::PreOrder(lpfnForEach<Args...> lpfn, Size level, Args... args)
{
       for( Size i = 0 ; i < m_KeyCount ; i++) {
               lpfn(m_Keys[i], level, args...);
       }
...
template <typename keyType, typename ObjIDType>
template <typename... Args>
void TwoThreeTreePage<keyType, ObjIDType>::PostOrder(lpfnForEach<Args...> lpfn, Size level, Args... args)
{
       for( Size i = 0 ; i <= m_KeyCount ; i++) {
               if( m_SubPages[i] )
...
template <typename keyType, typename ObjIDType>
template <typename... Args>
typename TwoThreeTreePage<keyType, ObjIDType>::ObjectInfo *
TwoThreeTreePage<keyType, ObjIDType>::FirstThat(lpfnFirstThat<Args...> lpfn, Size level, Args... args)
{
       ObjectInfo *pTmp;
       for( Size i = 0 ; i < m_KeyCount ; i++){
               if( m_SubPages[i] ){
               ...
               if( lpfn(m_Keys[i], level, args...) ) return &m_Keys[i];
       }
        if( m_SubPages[m_KeyCount] )
        {       
...
template <typename keyType, typename ObjIDType>
tt_ErrorCode TwoThreeTreePage<keyType, ObjIDType>::Remove(const keyType &key, const ObjIDType ObjID)
{
       tt_ErrorCode error = tt_ok;
       Size pos = tt_binary_search(m_Keys, 0, m_KeyCount, key);
       if( pos < NumberOfKeys() && key == m_Keys[pos].key ) 
       {
               if( !m_SubPages[pos+1] )  
               {
                       ::tt_remove(m_Keys, pos);
                       NumberOfKeys()--;
                       if( Underflow() ) return tt_underflow;
                       return tt_ok;
               }
       ...
       else if( pos == NumberOfKeys() ) error = m_SubPages[pos]->Remove(key, ObjID);
       else if( key <= m_Keys[pos].key ){ 
               if( m_SubPages[pos] ) error = m_SubPages[pos]->Remove(key, ObjID);
               else return tt_nofound;
       }
       
       if( error == tt_underflow ){
               if( TreatUnderflow(pos) ) return tt_ok;
               if( IsRoot() && NumberOfKeys() == 2 ) return MergeRoot();
               return Merge(pos);
       }
       if( error == tt_nofound ) return tt_nofound;
       return tt_ok;
}

template <typename keyType, typename ObjIDType>
tt_ErrorCode TwoThreeTreePage<keyType, ObjIDType>::Merge(Size pos)
{
...
template <typename keyType, typename ObjIDType>
tt_ErrorCode TwoThreeTreePage<keyType, ObjIDType>::MergeRoot()
{
       Size pos = 1;
       assert( m_SubPages[pos-1]->NumberOfKeys() +
                       m_SubPages[ pos ]->NumberOfKeys() +
       ...
       return tt_rootmerged;
}

template <typename keyType, typename ObjIDType>
typename TwoThreeTreePage<keyType, ObjIDType>::ObjectInfo &
TwoThreeTreePage<keyType, ObjIDType>::GetFirstObjectInfo()
{
...
template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::Create()
{
       Reset();
...
template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::Reset()
{
       for( Size i = 0 ; i < m_KeyCount ; i++ )
               delete m_SubPages[i];
       clear();
...
template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::clear()
{
       m_KeyCount = 0;
}

template <typename keyType, typename ObjIDType>
TwoThreeTreePage<keyType, ObjIDType> * CreateBTreeNode (Size maxKeys, bool unique)
{
       return new TwoThreeTreePage<keyType, ObjIDType> (maxKeys, unique);
}

template <typename keyType, typename ObjIDType>
void TwoThreeTreePage<keyType, ObjIDType>::MovePage(BTPage *pChildPage, vector<ObjectInfo> &tmpKeys,vector<BTPage *> &tmpSubPages)
{
...
template <typename keyType, typename ObjIDType>
Size TwoThreeTreePage<keyType, ObjIDType>::GetFreeCellsOnLeft(Size pos)
{
...
template <typename keyType, typename ObjIDType>
Size TwoThreeTreePage<keyType, ObjIDType>::GetFreeCellsOnRight(Size pos)
{
