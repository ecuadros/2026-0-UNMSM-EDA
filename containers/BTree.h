...
public:
       ...
       Size size() { lock_guard<mutex> lock(m_mutex); return m_NumKeys; }

       Size height() { lock_guard<mutex> lock(m_mutex); return m_Height; }

       Size GetOrder() { lock_guard<mutex> lock(m_mutex); return m_Order; }
       ...
       void InOrder(Func func, Args&&... args) {
              m_Root.InOrder(func, 0, forward<Args>(args)...);
       }

       template <typename Func, typename... Args>
       void PreOrder(Func func, Args&&... args) {
              m_Root.PreOrder(func, 0, forward<Args>(args)...);
       }

       template <typename Func, typename... Args>
       void PostOrder(Func func, Args&&... args) {
              m_Root.PostOrder(func, 0, forward<Args>(args)...);
       }

       template <typename Func, typename... Args>
       void Foreach(Func func, Args&&... args) {
              InOrder(func, forward<Args>(args)...);
       }
