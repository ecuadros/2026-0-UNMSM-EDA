...
typedef int T1;
template <class keyType, class ObjIDType, class Traits>
class BTree;
...
template <class keyType, class ObjIDType>
class CBTreePage
{
    friend class BTree<keyType, ObjIDType, BTreeTraits<keyType>>;

    typedef CBTreePage<keyType, ObjIDType>    BTPage;
    typedef tagObjectInfo<keyType, ObjIDType> ObjectInfo;

    typedef void       (*lpfnForEach2)   (ObjectInfo& info,  T1 level, void* pExtra1);
    typedef void       (*lpfnForEach3)   (ObjectInfo& info,  T1 level, void* pExtra1, void* pExtra2);
    typedef ObjectInfo*(*lpfnFirstThat2) (ObjectInfo& info,  T1 level, void* pExtra1);
    typedef ObjectInfo*(*lpfnFirstThat3) (ObjectInfo& info,  T1 level, void* pExtra1, void* pExtra2);

public:
    CBTreePage( T1 maxKeys, bool unique = true);
    CBTreePage(CBTreePage&& other) noexcept;     // Move constructor para BTree move
    virtual ~CBTreePage();
    ...
    // Inorden
    void ForEach(lpfnForEach2 lpfn,  T1 level, void* pExtra1);
    void ForEach(lpfnForEach3 lpfn,  T1 level, void* pExtra1, void* pExtra2);

    // Preorden  
    void ForEachPreOrder(lpfnForEach2 lpfn,  T1 level, void* pExtra1);
    void ForEachPreOrder(lpfnForEach3 lpfn,  T1 level, void* pExtra1, void* pExtra2);

    // Postorden 
    void ForEachPostOrder(lpfnForEach2 lpfn,  T1 level, void* pExtra1);
    void ForEachPostOrder(lpfnForEach3 lpfn,  T1 level, void* pExtra1, void* pExtra2);

    ObjectInfo* FirstThat(lpfnFirstThat2 lpfn,  T1 level, void* pExtra1);
    ObjectInfo* FirstThat(lpfnFirstThat3 lpfn,  T1 level, void* pExtra1, void* pExtra2);
     T1 GetNumberOfKeys() const { return m_KeyCount; }

protected:
     T1  m_MinKeys;
     T1  m_MaxKeys;
     T1  m_MaxKeysForChilds;
    bool m_Unique;
    bool m_isRoot;
...
template <typename keyType, typename ObjIDType>
CBTreePage<keyType, ObjIDType>::CBTreePage( T1 maxKeys, bool unique)
...
// Move Constructor
template <typename keyType, typename ObjIDType>
CBTreePage<keyType, ObjIDType>::CBTreePage(CBTreePage&& other) noexcept
...
// ForEach inorden 
template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::ForEach(lpfnForEach2 lpfn,  T1 level, void* pExtra1)
{
    for ( T1 i = 0; i < m_KeyCount; i++) {
        if (m_SubPages[i]) m_SubPages[i]->ForEach(lpfn, level+1, pExtra1);
        lpfn(m_Keys[i], level, pExtra1);
    }
    if (m_SubPages[m_KeyCount]) m_SubPages[m_KeyCount]->ForEach(lpfn, level+1, pExtra1);
}

template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::ForEach(lpfnForEach3 lpfn,  T1 level, void* pExtra1, void* pExtra2)
{
    for ( T1 i = 0; i < m_KeyCount; i++) {
        if (m_SubPages[i]) m_SubPages[i]->ForEach(lpfn, level+1, pExtra1, pExtra2);
        lpfn(m_Keys[i], level, pExtra1, pExtra2);
    }
    if (m_SubPages[m_KeyCount]) m_SubPages[m_KeyCount]->ForEach(lpfn, level+1, pExtra1, pExtra2);
}

// ForEach preorden 
template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::ForEachPreOrder(lpfnForEach2 lpfn,  T1 level, void* pExtra1)
{
    for ( T1 i = 0; i < m_KeyCount; i++) {
        lpfn(m_Keys[i], level, pExtra1);               // Visita primero
        if (m_SubPages[i]) m_SubPages[i]->ForEachPreOrder(lpfn, level+1, pExtra1);
    }
    if (m_SubPages[m_KeyCount]) m_SubPages[m_KeyCount]->ForEachPreOrder(lpfn, level+1, pExtra1);
}

template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::ForEachPreOrder(lpfnForEach3 lpfn,  T1 level, void* pExtra1, void* pExtra2)
{
    for ( T1 i = 0; i < m_KeyCount; i++) {
        lpfn(m_Keys[i], level, pExtra1, pExtra2);
        if (m_SubPages[i]) m_SubPages[i]->ForEachPreOrder(lpfn, level+1, pExtra1, pExtra2);
    }
    if (m_SubPages[m_KeyCount]) m_SubPages[m_KeyCount]->ForEachPreOrder(lpfn, level+1, pExtra1, pExtra2);
}

// ForEach postorden
template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::ForEachPostOrder(lpfnForEach2 lpfn,  T1 level, void* pExtra1)
{
    for ( T1 i = 0; i <= m_KeyCount; i++)
        if (m_SubPages[i]) m_SubPages[i]->ForEachPostOrder(lpfn, level+1, pExtra1);
    for ( T1 i = 0; i < m_KeyCount; i++)
        lpfn(m_Keys[i], level, pExtra1);
}

template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::ForEachPostOrder(lpfnForEach3 lpfn,  T1 level, void* pExtra1, void* pExtra2)
{
    for ( T1 i = 0; i <= m_KeyCount; i++)
        if (m_SubPages[i]) m_SubPages[i]->ForEachPostOrder(lpfn, level+1, pExtra1, pExtra2);
    for ( T1 i = 0; i < m_KeyCount; i++)
        lpfn(m_Keys[i], level, pExtra1, pExtra2);
}
...
CBTreePage<keyType, ObjIDType>::FirstThat(lpfnFirstThat2 lpfn,  T1 level, void* pExtra1)
{
    ObjectInfo* pTmp;
    for ( T1 i = 0; i < m_KeyCount; i++) {
        if (m_SubPages[i]) {
        ...
        if (lpfn(m_Keys[i], level, pExtra1)) return &m_Keys[i];
    }
...
template <typename keyType, typename ObjIDType>
typename CBTreePage<keyType, ObjIDType>::ObjectInfo*
CBTreePage<keyType, ObjIDType>::FirstThat(lpfnFirstThat3 lpfn,  T1 level, void* pExtra1, void* pExtra2)
{
    ObjectInfo* pTmp;
    for ( T1 i = 0; i < m_KeyCount; i++) {
        if (m_SubPages[i]) {
        ...
        if (lpfn(m_Keys[i], level, pExtra1, pExtra2)) return &m_Keys[i];
    }
    if (m_SubPages[m_KeyCount]) {
...
// Print
template <typename keyType, typename ObjIDType>
void PrintHelper(tagObjectInfo<keyType, ObjIDType>& info,  T1 level, void* pExtra)
{
...
{
    lpfnForEach2 lpfn = &PrintHelper<keyType, ObjIDType>;
    ForEach(lpfn, 0, &os);
}
...
bool CBTreePage<keyType, ObjIDType>::Redistribute1(int& pos)
{
    if (m_SubPages[pos]->Underflow()) {
        ...
        if (nkol > nkor) {
            ...
            else if (pos == NumberOfKeys()) return (--pos, false);
            else return false;
        } else {
            if (m_SubPages[pos+1]->NumberOfKeys() > m_SubPages[pos+1]->MinNumberOfKeys())
         ...
         T1 fcol = GetFreeCellsOnLeft(pos), fcor = GetFreeCellsOnRight(pos);
...
template <typename keyType, typename ObjIDType>
bool CBTreePage<keyType, ObjIDType>::Redistribute2( T1 pos)
{
...
template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::RedistributeR2L( T1 pos)
{
...
template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::RedistributeL2R( T1 pos)
{
    BTPage* pSource = m_SubPages[pos], *pTarget = m_SubPages[pos+1];
    while (pSource->GetNumberOfKeys() > pSource->MinNumberOfKeys() &&
           pTarget->GetNumberOfKeys() < pSource->GetNumberOfKeys()) {
...
template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::SplitChild( T1 pos)
{
...
template <typename keyType, typename ObjIDType>
bt_ErrorCode CBTreePage<keyType, ObjIDType>::Merge( T1 pos)
{
    ...
    pChild3->Destroy();
     ...
     T1 nKeys = pChild1->GetFreeCells(), i = 0;
    for (; i < nKeys; i++) {
        pChild1->m_Keys[i]    = tmpKeys[i];
        pChild1->m_SubPages[i] = tmpSubPages[i];
        pChild1->NumberOfKeys()++;
...
template <typename keyType, typename ObjIDType>
void CBTreePage<keyType, ObjIDType>::MovePage(BTPage* pChildPage,
    vector<ObjectInfo>& tmpKeys, vector<BTPage*>& tmpSubPages)
{
     T1 nKeys = pChildPage->GetNumberOfKeys(), i = 0;
    for (; i < nKeys; i++) {
...
template <typename keyType, typename ObjIDType>
 T1 CBTreePage<keyType, ObjIDType>::GetFreeCellsOnLeft( T1 pos)
{
...
template <typename keyType, typename ObjIDType>
 T1 CBTreePage<keyType, ObjIDType>::GetFreeCellsOnRight( T1 pos)
{
