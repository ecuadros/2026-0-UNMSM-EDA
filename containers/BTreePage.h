...
class CBTreePage {
    ...
    typedef void (*lpfnForEach3)(ObjectInfo &info, Size level, void *pExtra1, void *pExtra2);
    typedef ObjectInfo* (*lpfnFirstThat3)(ObjectInfo &info, Size level, void *pExtra1, void *pExtra2);
...
public:
    ...
    void ForEach(lpfnForEach3 lpfn, Size level, void *pExtra1, void *pExtra2);
    ObjectInfo* FirstThat(lpfnFirstThat2 lpfn, Size level, void *pExtra1);
    ObjectInfo* FirstThat(lpfnFirstThat3 lpfn, Size level, void *pExtra1, void *pExtra2);
...
protected:
    ...
    bool  TreatUnderflow  (Size &pos) { return Redistribute1(pos) || Redistribute2(pos); }
...
void insert_at(Container& container, const ObjType &object, Size pos) {
    ...
    for(Size i = size-2 ; i >= pos ; i--) container[i+1] = container[i];
...
void remove(Container& container, Size pos) {
    Size size = container.size();
    for(Size i = pos+1 ; i < size ; i++) container[i-1] = container[i];
}
      ...
      m_isRoot(another.m_isRoot), m_KeyCount(another.m_KeyCount) 
{
    ...
    for (Size i = 0; i <= m_KeyCount; i++) {
        if (another.m_SubPages[i]) m_SubPages[i] = new CBTreePage<Traits>(*another.m_SubPages[i]);
    }
}
...
CBTreePage<Traits>::CBTreePage(CBTreePage<Traits>&& another) noexcept
    : m_MinKeys(another.m_MinKeys), m_MaxKeys(another.m_MaxKeys),
      m_MaxKeysForChilds(another.m_MaxKeysForChilds), m_Unique(another.m_Unique),
      m_isRoot(another.m_isRoot), m_KeyCount(another.m_KeyCount),
      m_Keys(move(another.m_Keys)), m_SubPages(move(another.m_SubPages))
{ another.m_KeyCount = 0;}

template <typename Traits>
bt_ErrorCode CBTreePage<Traits>::Insert(const value_type& key, const ref_type ObjID) {
    bool found = false;
...
bool CBTreePage<Traits>::Redistribute1(Size &pos) {
    if( m_SubPages[pos]->Underflow() ) {
        ...
        if( nkol > nkor ) {
            ...
            else { if( pos == NumberOfKeys() ) return (--pos, false); else return false; }
        } else {
            if( m_SubPages[pos+1]->NumberOfKeys() > m_SubPages[pos+1]->MinNumberOfKeys() ) RedistributeR2L(pos+1);
            else { if( pos == 0 ) return (++pos, false); else return false; }
        }
    } else {
        Size fcol = GetFreeCellsOnLeft(pos), fcor = GetFreeCellsOnRight(pos);
...
template <typename Traits>
void CBTreePage<Traits>::SplitChild(Size pos) {
...
template <typename Traits>
void CBTreePage<Traits>::SplitPageInto3(vector<ObjectInfo>& tmpKeys, vector<BTPage *>& tmpSubPages,
                                        BTPage*& pChild1, BTPage*& pChild2, BTPage*& pChild3,
                                        ObjectInfo& oi1, ObjectInfo& oi2) {
...
template <typename Traits>
void CBTreePage<Traits>::ForEach(lpfnForEach2 lpfn, Size level, void *pExtra1) {
    for(Size i = 0 ; i < m_KeyCount ; i++) {
...
template <typename Traits>
void CBTreePage<Traits>::ForEach(lpfnForEach3 lpfn, Size level, void *pExtra1, void *pExtra2) {
    for(Size i = 0 ; i < m_KeyCount ; i++) {
...
template <typename Traits>
typename CBTreePage<Traits>::ObjectInfo* CBTreePage<Traits>::FirstThat(lpfnFirstThat2 lpfn, Size level, void *pExtra1) {
    ObjectInfo *pTmp;
    for(Size i = 0 ; i < m_KeyCount ; i++) {
        if( m_SubPages[i] ){
...
template <typename Traits>
typename CBTreePage<Traits>::ObjectInfo* CBTreePage<Traits>::FirstThat(lpfnFirstThat3 lpfn, Size level, void *pExtra1, void *pExtra2) {
    ObjectInfo *pTmp;
    for(Size i = 0 ; i < m_KeyCount ; i++){
        if( m_SubPages[i] ){
...
template <typename Traits>
typename CBTreePage<Traits>::ObjectInfo & CBTreePage<Traits>::GetFirstObjectInfo() {
...
void CBTreePage<Traits>::Reset() {
    if (m_SubPages.empty()) { clear(); return; }
    for( Size i = 0 ; i <= m_KeyCount ; i++ ) {
        if (m_SubPages[i]) delete m_SubPages[i];
    }
    for( Size i = 0 ; i < (Size)m_SubPages.size() ; i++ ) m_SubPages[i] = nullptr;
    clear();
}
