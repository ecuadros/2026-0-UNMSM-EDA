...
struct tagObjectInfo {
        ...
        Size GetUseCounter () const { return UseCounter; }
...
class CBTreePage {
        ...
        void InOrder(Func fn, Size level, Args&&... args) {
                for (Size i = 0; i < m_KeyCount; i++) {
                        if (m_SubPages[i]) 
                                m_SubPages[i]->InOrder(fn, level + 1, forward<Args>(args)...);
                        
                        fn(m_Keys[i], level, forward<Args>(args)...);
                }
                if (m_SubPages[m_KeyCount])
                        m_SubPages[m_KeyCount]->InOrder(fn, level + 1, forward<Args>(args)...);
        }
        ...
        void PreOrder(Func fn, Size level, Args&&... args) {
                ...
                for (Size i = 0; i <= m_KeyCount; i++) {
                        if (m_SubPages[i]) 
                                m_SubPages[i]->PreOrder(fn, level + 1, forward<Args>(args)...);
                }
        }
        ...
        void PostOrder(Func fn, Size level, Args&&... args) {
                for (Size i = 0; i <= m_KeyCount; i++) {
                        if (m_SubPages[i]) 
                                m_SubPages[i]->PostOrder(fn, level + 1, forward<Args>(args)...);
                }
                for (Size i = 0; i < m_KeyCount; i++) {
                        fn(m_Keys[i], level, forward<Args>(args)...);
                }
        ...
        template <typename Func, typename... Args>
        ObjectInfo* FirstThat(Func fn, Size level, Args&&... args) {
                        ObjectInfo* pTmp = nullptr;
                for (Size i = 0; i < m_KeyCount; i++) {
                        if (m_SubPages[i]) {
                                pTmp = m_SubPages[i]->FirstThat(fn, level + 1, forward<Args>(args)...);
                                if (pTmp) return pTmp;
                        }
                        if (fn(m_Keys[i], level, forward<Args>(args)...)) 
                                return &m_Keys[i];
                }
                if (m_SubPages[m_KeyCount]) {
                        pTmp = m_SubPages[m_KeyCount]->FirstThat(fn, level + 1, forward<Args>(args)...);
                        if (pTmp) return pTmp;
                }
                return nullptr;
        }
 ...
 public:
        ...
        Size SearchInPage(const value_type &key, bool &found) {
                ...
                found = (pos < m_KeyCount && key == m_Keys[pos].key);
                return pos;
        }
...
Size binary_search(Container& container, Size first, Size last, const ObjType &object) {
...
template <typename Traits>
CBTreePage<Traits>::CBTreePage(Size maxKeys, bool unique)
