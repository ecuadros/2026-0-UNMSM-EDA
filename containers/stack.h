...
public:
    ...
    CStack(const CStack &another) {
        const Node *iter = another.SLast;
        // Recolectamos los elementos en orden inverso
        vector<pair<T, ref_type>> temp;
        while (iter) {
            temp.push_back({iter->GetValue(), iter->GetRef()});
            iter = iter->GetNext();
        }
        // Los insertamos al revés para mantener el orden de la pila
        for (int i = temp.size() - 1; i >= 0; i--)
            push(temp[i].first, temp[i].second);
    }
