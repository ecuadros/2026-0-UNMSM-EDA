...
static bool Mult7_Double(T1 &x){
    return x % 7 == 0;
}
static bool GreaterThan_Double(T1 &x, T1 limit){
    return x > limit;
}
static bool DivisibleBy_Double(T1 &x, T1 d){
    return d != 0 && (x % d == 0);
}

// Para Foreach (miembro)
static void AddConst_Double(T1 &x, T1 k){
    x += k;
}
} // namespace

void DemoDoubleLinkedLists(){
    cout << "\n=== DemoCDoubleLinkedList started ===\n";

    // ------------------------------------------------------------
    // [A] Base: Insert + push_back + operator<< + archivo (<<)
    // ------------------------------------------------------------
    cout << "\n[A] Base: Insert + push_back + operator<< + archivo\n";
    CDoubleLinkedList< AscendingTrait<T1> > a;
    a.Insert(20,5);
    a.Insert(30,3);
    a.push_back(40,4); // push_back engancha al final (independiente del orden)
    cout << "Ready to print a:\n" << a << endl;

    ofstream of("double_list_base.txt");
    of << a;
    of.close();
    cout << "double_list_base.txt written\n";
    ...
    {
        CDoubleLinkedList< AscendingTrait<T1> > tmp;
        tmp.Insert(20,5);
        tmp.Insert(30,3);
        tmp.push_back(40,4);
    ...
    CDoubleLinkedList< AscendingTrait<T1> > c;
    c.Insert(10,1);
    c.Insert(20,2);
    cout << "Antes clear: " << c;
    c.clear();
    ...
    CDoubleLinkedList< AscendingTrait<T1> > b(a);
    cout << "Original a: " << a;
    cout << "Copy b(a):  " << b;
    a.push_back(99,9);
    ...
    CDoubleLinkedList< AscendingTrait<T1> > e1;
    ...
    // ------------------------------------------------------------
    cout << "\n[G] Test operator[]\n";
    CDoubleLinkedList< AscendingTrait<T1> > idx;
    idx.Insert(10,1);
    idx.Insert(20,2);
    idx.Insert(30,3);
    cout << "Before: " << idx;
    ...
    CDoubleLinkedList< DescendingTrait<T1> > desc;
    desc.Insert(20,1);
    desc.Insert(10,2);
    desc.Insert(30,3);
    ...
    CDoubleLinkedList< AscendingTrait<T1> > it;
    it.Insert(20,1);
    it.Insert(10,2);
    it.Insert(30,3);
    ...
    // ------------------------------------------------------------
    // [I2] Backward iterator: rbegin/rend (lo NUEVO de LDE)
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // [J] FirstThat (variadic, como profe)  [AZUL]
    // ------------------------------------------------------------
    cout << "\n[J] Test FirstThat (variadic, como profe)\n";
    CDoubleLinkedList< AscendingTrait<T1> > ft;
    ft.Insert(10,2);
    ft.Insert(14,4);
    ft.Insert(20,1);
    ft.Insert(30,3);
    ...
    auto i1 = ft.FirstThat(&Mult7_Double);
    if(i1 != ft.end()) cout << "First mult of 7: " << *i1 << endl;

    auto i2 = ft.FirstThat(&GreaterThan_Double, 25);
    if(i2 != ft.end()) cout << "First > 25: " << *i2 << endl;

    auto i3 = ft.FirstThat(&DivisibleBy_Double, 5);
    ...
    // ------------------------------------------------------------
    // [K] Foreach (miembro) - modifica elementos  [AZUL]
    // ------------------------------------------------------------
    cout << "\n[K] Foreach (member)\n";
    CDoubleLinkedList< AscendingTrait<T1> > fx;
    fx.Insert(10,1);
    fx.Insert(20,2);
    fx.Insert(30,3);
    cout << "Before Foreach(AddConst,+5): " << fx;
    fx.Foreach(&AddConst_Double, 5);
    ...
    CDoubleLinkedList< AscendingTrait<T1> > read;
    ifstream inf("double_list_base.txt");
    if(!inf){
        cout << "No se pudo abrir double_list_base.txt\n";
    }else{
        inf >> read;
        cout << "Read double_list_base.txt -> read: " << read;
    }

    cout << "\n=== DemoCDoubleLinkedList finished ===\n";
}
